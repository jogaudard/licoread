...
allobs <- read_lines(file)
# read_yaml("raw_files/header.81x")
# create new cols with the elements from yamal and footer

first_el <- substr(allobs, start = 1, stop = 1)
...
line_skip <- min(data_loc) - 1

read_all <- read_tsv(file, skip = line_skip)
View(read_all)

data_loc_corr <- data_loc - line_skip
...
metadata <- read_yaml(text = allobs[-data_loc])
View(allobs[-data_loc])
View(data_loc)

View(as.numeric(zero_lines))

metameta <- tibble(
    col_meta = as.double(zero_lines)
) |>
rowid_to_column("ID")
...
data_loc_tbl <- tibble(
    col_data = as.double(data_loc)
)
View(data_loc_tbl)

# we need a vector of position of lines with col names
metaID <- full_join(metameta, data_loc_tbl, by = join_by(col_meta == col_data)) |>
arrange(col_meta) |>
...
header_loc <- as.vector(metaID$min) - 1

meta_antiloc <- append(data_loc, header_loc)
metadata <- read_yaml(text = allobs[-meta_antiloc])

test <- bind_rows(metameta, data_loc_tbl) |>
arrange(col) |>
fill(ID, .direction = "up")
View(test)
